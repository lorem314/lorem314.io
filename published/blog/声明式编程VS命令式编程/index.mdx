---
id: "bebfd458151e1b6aba71b2c863b4433c"
...
本文通过编写一个的 Demo —— 在控制台中每秒输出当前时间，来比较命令式编程和声明式编程的不同。

## 函数式编程

声明式编程属于函数式编程里的一类。从其名字中就可以看出，这类编程需要大量地依赖和使用函数，定义一个函数就像定义一个变量一样平常，倒不如说在 JS 中，函数就可以看作一个变量。

其原因是，JS 中的函数是一等公民`(first-class citizen)`，这意味着函数可以被看作成变量，被传入另一个函数，或被函数当作值返回使用。有了 ES6 语法的加持，使用箭头函数、Promise 和剩余参数可以极大提高函数式编程的使用效率。

## 命令式 VS 声明式

声明式编程倾向于描述程序执行的顺序，或是说按照何种顺序执行，而非定义其该如何发生。

我们先来看一个例子：将一个字符串转化为 URL 兼容字符串。
...
我们使用一个 for 循环来遍历字符串，如果是普通字母则不做处理，原封不动地加到 urlFriendly 字符串的末尾，如果是空格，则添加连字符。
...
每秒钟，先清除控制台，再在控制台中输出当前时间。时间的格式为 12 小时制，数字为一位数时要加上前缀 0，且要在末尾加上 am 或 pm 来表示上午还是下午。

如：上午 `05:26:52 am`，下午 `09:12:55 pm`
...
使用 setInterval，每秒执行一次 logClockTime 函数，该函数负责在控制台中输出时间。

```js
function logClockTime() {
  var time = getClockTime() // 获取时间
  console.clear() // 清除控制台并输出时间
...
接下来是声明式编程的写法。之前说过，声明式编程非常依赖函数，我们先来定义一些函数：
...
目前我们完成了所有的函数，下面我们将这些函数整合到一起。在此，我们引入 compose 函数
...
我们使用 compose 函数来优雅地组合所有其他的函数，避免了定义中间变量，和嵌套调用的写法引起的嵌套地狱。
