...
<Figure id="figure_3-1" title="图片 3-1 cluster 的主从关系">

![图片 3-1 cluster 的主从关系](./figure_3-1.png)
...
这种不必要的、长时间的粘性连接，可以通过<Link to="../第2章#基于-RPC-的-gRPC">2.3 基于 RPC 的 gRPC</Link>中代码来得到重现。无需修改生产者和消费者的服务代码，通过<a href="#code_3-1">代码 3-1</a>中的通用 cluster 主进程来生成 gRPC 子进程，即可重现问题。执行生产者主进程和消费者，向消费者服务发送几个请求，响应中的 `producer_data.pid` 的值将总会是同一个。
...
<Figure id="figure_3-2" title="图片 3-2 反向代理拦截传入的网络流量">

![图片 3-2 反向代理拦截传入的网络流量](./figure_3-2.png)
...
```cfg id=3-3 title=haproxy/stats.cfg
frontend inbound
mode http
bind localhost:8000
stats enable
...
技术上讲，使用 HAProxy 负载均衡无需对应用程序进行任何改动。然而，为了能更好的展示，我们添加一个名为健康检查的功能。目前，使用一个响应代码为 200 的后端节点作为健康标准则足矣。复制 web-api/consumer-http-basic.js 中的代码，并添加一个新的端点，如<a href="#code_3-4">代码 3-4</a>所示。<Link to="../第4章#健康检查">4.5 健康检查</Link>将着眼于构建一个更准确的健康检查端点。

<Figure id="figure_3-3" title="图片 3-3 HAProxy 负载均衡">

![图片 3-3 HAProxy 负载均衡](./figure_3-3.png)
...
尽管可以被配置得非常频繁地进行健康检查，但这并不是检测服务器何时关闭地最佳解决方案。使用这种方法总是会存在将请求发往不健康服务器的风险。第 8 章中的<Link to="../第8章#等幂性和消息传递弹性">8.6 等幂性和消息传递弹性</Link>着眼于解决此问题，客户端需要被配置为重试失败的请求。

### 压缩

通过在包含 HAProxy 应压缩内容的特定后端上设置附加参数，就可以使 HAProxy 开启压缩。开启了压缩的配置文件如代码 3-6 中的内容所示。

```rust id=3-6 title=haproxy/compression.cfg
defaults
mode http
...
<Figure id="figure_3-4" title="图片 3-4 HAProxy 的 TLS 终止">

![图片 3-4 HAProxy 的 TLS 终止](./figure_3-4.png)

</Figure>

使用 HAProxy 进行 TLS 终止相当简单，第 2 章中的<Link to="../第2章#HTTPS-TLS">2.1.4 HTTPS/TLS</Link>中的规则仍然适用。例如，所有证书生成和信任链的概念仍然适用，并且这些证书文件遵循易于理解的标准。唯一的区别是，在这一小节中，我们用到了 `.pem` 文件，该文件同时包含 `.cert` 文件和 `.key` 文件中的内容。代码 3-7 是之前命令的一个改动版本，它将生成明匙和密匙文件，并生成将两者连在一起 `.pem` 文件。
...
```rust id=3-8 title=haproxy/tls.cfg
defaults
  mode http
  timeout connect 5000ms
  timeout client 50000ms
...
```rust id=3-10 title=haproxy/backpressure.cfg
defaults
  maxconn 8 // 1
  mode http
...
<Figure id="figure_3-5" title="图片 3-5 Autocannon 延迟结果图">

![图片 3-5 Autocannon 延迟结果图](./figure_3-5.png)
...
```rust id=3-12 title=haproxy/benchmark-basic.cfg
defaults
  mode http
...
<Figure id="figure_3-6" title="图片 3-6 HAProxy 延迟">

![图片 3-6 HAProxy 延迟](./figure_3-6.png)
...
```rust id=3-13 title=haproxy/passthru.cfg
defaults
  mode tcp
  timeout connect 5000ms
  timeout client 50000ms
...
<Figure id="figure_3-7" title="图片 3-7 NodeJS gzip 压缩延迟">

![图片 3-7 NodeJS gzip 压缩延迟](./figure_3-7.png)

</Figure>

此测试表明，使用 HAProxy 执行 gzip 压缩时的请求处理速度比使用 Node.js 时要快一些。

<Figure id="figure_3-8" title="图片 3-8 HAProxy gzip 压缩延迟">

![图片 3-8 HAProxy gzip 压缩延迟](./figure_3-8.png)
...
<Figure id="figure_3-9" title="图片 3-9 云端的基准测试">

![图片 3-9 云端的基准测试](./figure_3-9.png)
...
<Figure id="figure_3-10" title="图片 3-10 基于 HTTP 的 JSON 的基准测试">

![图片 3-10 基于 HTTP 的 JSON 的基准测试](./figure_3-10.png)
...
<Figure id="figure_3-11" title="图片 3-11 GraphQL 基准测试">

![图片 3-11 GraphQL 基准测试](./figure_3-11.png)
...
<Figure id="figure_3-12" title="图片 3-12 GraphQL 基准测试">

![图片 3-12 GraphQL 基准测试](./figure_3-12.png)
...
<Figure id="figure_3-13" title="图片 3-13 对生产级别应用程序进行基准测试">

![图片 3-13 对生产级别应用程序进行基准测试](./figure_3-13.png)
...
```rust id=3-14 title=haproxy/fibonacci.cfg
defaults
  mode http
