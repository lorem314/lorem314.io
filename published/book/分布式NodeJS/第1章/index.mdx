...
import { Note, Tip, Warning, Figure, BorderBox } from "#html/oreilly"
...
JavaScript 语言正在从**单线程**语言转为**多线程**语言。例如 `Atomics` 对象，提供了与不同**线程**(thread)通信的机制，而 `SharedArrayBuffer` 可被跨线程地读取和存储。在撰写本书的时间节点上，多线程 JavaScript 还未迎合大众，在社区中流行开来。如今的 JavaScript 已经是多线程，但该语言的核心以及生态，仍被认为是单线程。
...
<Tip>
  调用栈层数的最大上限一般由 JavaScript 引擎决定。例如，第 14 版本的 NodeJS
  所使用的 JavaScript 引擎 V8 的调用栈层数的最大上限是 15,000。
</Tip>
...
JavaScript 通过**事件循环**(event loop)的方式来处理并行任务。本章的<a href="#NodeJS-中的事件循环">1.3 NodeJS 中的事件循环</a>一节中会更详细地介绍有关事件循环的机制，至于现在，我们可以将其抽象为一个**不断检查是否有未完成的任务要去完成的**死循环。当检查到有需要完成的任务时，它便开始去执行任务——这里之的任务通常是一个函数，并且是在一个新的调用栈中执行该函数。完成任务后，事件循环又会回到不断检查的状态，直到再次检查到有任务要完成。

我们通过示例代码 1-1 来具体说明一下事件循环的过程。首先执行 setTimeout 函数，将 `x()` 函数的运行放到宏队列中，之后在当前栈中执行函数 `a()` 。执行完函数 a 后，当前栈中的任务均已被执行，事件循环便开始检查是否有未完成的任务，事件循环只有在一个栈中的所有任务都执行完毕后，才会去检查其他的栈。接下来开始执行被放到宏队列中的 x 函数。

```js id=1-1 title=多个JavaScript堆栈示例
function a() {
  b()
}
function b() {
...
<a href="#figure_1-1">图示 1-1</a> 是上述代码示例的可视化。图中有两个独立的调用栈，并且每个调用栈的层数会随着其内部有函数被调用而增高。

<Figure id="figure_1-1" title="图示 1-1 多个 JavaScript 堆栈的可视化">

![图示 1-1 多个 JavaScript 堆栈的可视化](./figure_1-1.png)
...
下面这个问题我常出给来面试的人，示例代码 1-2 会输出什么结果。另外，它们大概会在第几毫秒时输出。

<div class="custom-code-title">示例代码 1-2 JavaScript 计时问题</div>
...
<blockquote>
  扩展(scale)：保持程序的行为不变，增加可负载量来处理大量的用户请求。
  <a
    href="https://www.geeksforgeeks.org/overview-of-scaling-vertical-and-horizontal-scaling/"
    target="_blank"
  >
...
目前有很多工具可用于更方便地管理应用程序的多个副本。<Link to="../第3章/#Cluster-模块">3.1 Cluster 模块</Link>一节中，将使用 NodeJS 内置的 cluster 模块将传入的 HTTP 请求发送到不同的应用程序实例副本。另一个内置模块 worker_threads 也能使多个 JavaScript 实例同时运行，child_process 模块可以用来生成并管理 NodeJS 进程。
...
通过使用 `worker_threads` 模块，是可以实现在两个不同的 JavaScript
实例中共享变量的。可通过创建 `SharedArrayBuffer` 实例，并用
`postMessage(value)`
方法传递数据给其他进程，数据可以字节数组的方式可被多个线程同时读写。
...
下面的表格 1-1 提供了之前面试问题的答案。

<div class="custom-code-title">表格 1-1 面试问题的答案和分析</div>
...
需要花费较长时间的这一类任务，是典型的读写(I/O)任务。例如，应用程序需要执行两个任务，任务 A 是从硬盘中读取文件中内容，任务 B 是向第三方服务器发送 HTTP 请求。如果某个操作需要两个任务同时进行 —— 如向 HTTP 请求回复 —— 则应用程序可以并行执行两个任务，如<a href="#figure_1-2">图示 1-2</a>(右)。如果任务 AB 无法同时被执行，或是它们必须被先后执行 —— 则向 HTTP 回复的总时间会变得稍长。

<Figure id="figure_1-2" title="图示 1-2 顺序与并行 IO 的可视化">

![图示 1-2 顺序与并行 IO 的可视化](./figure_1-2.png)

</Figure>

乍一看，这好像违反了 JavaScript 单线程的本质。NodeJS 应用怎么能够同时又读文件又发送 HTTP 请求呢？

这也正是其有趣之处。NodeJS 本身是**多线程**的，其底层由 C++ 语言实现，其中包含了第三方工具如 libuv，它主要处理操作系统抽象和读写操作，和 v8(JavaScript 引擎) 及其他第三方模块。再上一层，NodeJS 捆绑层，也存在一些 C++代码。只有 NodeJS 的最高层，是用 JavaScript 语言编写的，如 NodeJS 的部分 API 和 由其他用户编写并维护的 npm 模块的代码。<a href="#figure_1-3">图示 1-3</a> 展示了不同层级间的关系。

<Figure id="figure_1-3" title="图示 1-3 NodeJS的层级">

![图示 1-3 NodeJS 的层级](./figure_1-3.png)

</Figure>

在内部，libuv 维护一个线程池来管理 I/O 操作，以及像 crypto 和 zlib 这样的 CPU 密集型操作。这是一个有限大小的池，允许发生 I/O 操作。如果池只包含四个线程，则只能同时读取四个文件。代码示例 1-3 中，应用程序尝试读取文件，执行一些其他工作，然后处理文件内容。尽管应用程序中的 JavaScript 代码能够运行，但 Node.js 内部的一个线程正忙于将文件的内容从磁盘读取到内存中。

<div class="custom-code-title">示例 1-3 Nodejs 线程</div>

```js title=NodeJS线程 array=['32','str'] id=1-3
const fs = require("fs")
...
NodeJS API 中有一个常见模式，可以将此类对象配置为不再使进程保持活动状态。其中一些比其他更明显。例如，如果一个监听 HTTP 服务器端口被关闭，那么进程可能会选择结束。此外，其中许多对象都附加了一对方法，`unref()` 和 `ref()`。前者用于告诉对象不再保持进程处于活动状态，后者则相反。代码示例 1-4 演示了这种情况。

<div class="custom-code-title">代码示例 1-4 常见的ref()和unref()方法</div>
...
无论是运行在浏览器环境下的 JavaScript 还是 Node 环境下的 JavaScript，都有事件循环机制。相同之处在于，它们都被用于在不同的调用栈中处理异步任务地调度和执行；不同之处在于，浏览器中地事件循环经过优化，更好地驱动单页面应用程序，NodeJS 中的事件循环能够更好地在服务器上运行。该小节讲解 Node.js 中的事件循环。理解事件循环的基础不仅能更好的让我们了解应用程序中代码的执行先后，也可以避免将 bug 引入应用程序。
...
<Figure id="figure_1-4" title="图示 1-4 NodeJS 事件循环的不同阶段">

![图示 1-4 NodeJS 事件循环的不同阶段](./figure_1-4.png)
...
<div class="custom-code-title">event-loop-phases.js</div>
...
web-api 服务是 recipe-api 的下游，相反，recipe-api 是 web-api 的上游。 <a href="#figure_1-5">图示 1-5</a> 是这两个服务之间关系的可视化。

<Figure id="figure_1-5" title="图示 1-5 web-api 和 recipe-api 之间的关系">

![图示 1-5 web-api 和 recipe-api 之间的关系](./figure_1-5.png)
...
<div class="custom-code-title">示例 1-6 recipe-api/producer-http-basic.js</div>
...
<div class="custom-code-title">示例 1-7 web-api/consumer-http-basic.js</div>
