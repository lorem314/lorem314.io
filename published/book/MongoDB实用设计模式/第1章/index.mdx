...
设计一个新的应用程序时，首先要做的是设计数据结构。在像 MySQL 这种关系型数据库中，这一步骤对应为移除表格中的冗余，即**规范化**(normalization)。MongoDB，不同于关系型数据库，通过结构化的**文档**(document)来存储数据。关系型表格要求每个格子只能插入<u>一个单一</u>的数据，MongoDB BSON 文档则允许更加复杂的结构，如数组。
...
<div class="custom-table-title">表格1-1 电话簿v1</div>
...
机械硬盘在寻找数据的位置上所花费的时间，远比它读取数据花费的时间要多（如图 1-1）。例如，一张现代硬盘可能需要花费 5 毫秒(ms, 10^-3s)来找到数据在硬盘上的位置。当可以读取时，假设以 40 - 80MB 每秒的速度读取，电话簿表每行 1024bytes，那么，读取一行信息大概需要 12-25 微秒(μs, 10^-6s)。

![Untitled](%E7%AC%AC%201%20%E7%AB%A0%20%E5%B5%8C%E5%85%A5%E8%BF%98%E6%98%AF%E5%BC%95%E7%94%A8%20c6b7689a1aba4fbca8ce7daecd88c538/Untitled.png)
...
幸运的是，这对于应用程序设计者来说，在模式(schema)设计上开辟了新的可能。MongoDB 支持多值，我们能在不用担心冗余的情况下，享受到很多反范式设计下的性能提升。但又不幸的是，这又会复杂化模式设计。不同于在设计关系型数据库时可以遵循范式，MongoDB 中没有这种可以遵循的标准，大多数情况下的答案都是“看情况(it depends)”。
...
本章的目的在于，帮助你在不同的情况下，决定使用引用(referencing)还是嵌入(embedding)。
...
> In logic, mathematics, and computer science, arity (/ˈærɪti/) is **the number of arguments or operands taken by a function, operation or relation**. In mathematics, arity may also be called rank, but this word can have many other meanings. In logic and philosophy, arity may also be called adicity and degree.
...
def get_product_with_categories(product_id):
  ...
  category_ids = [
    p_c['category_id'] for p_c in db.product_category.find({
...
此时，获取产品信息的代码虽然看起来稍微复杂了一些，但避免了更新产品信息时，找出该产品在类别中所有曾出现过的地方这一麻烦操作了。
